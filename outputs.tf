...
output "swarm_managers" {
  value = "${concat(google_compute_instance.manager_follower.*.name, google_compute_instance.manager_follower.*.network_interface.0.access_config.0.assigned_nat_ip)}"
}
...
output "swarm_network_name" {
  value = "${google_compute_network.swarm.name}"
}